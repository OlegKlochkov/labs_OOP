...
public class BurningShip extends FractalGenerator{
    ...
    @Override
    public int numIterations(double x, double y) {//Реализует итеративную функцию для фрактала Burning Ship
        int iterationNum = 0;
        double real = 0;
        double imaginary = 0;
        while (iterationNum<MAX_ITERATIONS && real*real+imaginary*imaginary<4){
            double plhReal = real*real-imaginary*imaginary+x;
            double plhImaginary = 2*Math.abs(real)*Math.abs(imaginary)+y;
            real = plhReal;
